...
def compute_score_for_item(item, max_var, tokenizer, model, device):
    ...
    correct_final = correct_answer["ground_truth"]["target"]
    ...
    correct_emb = get_embedding(correct_final, tokenizer, model, device)
...
if __name__ == "__main__":
    ...
    input_file = f"difficulty_scripts/qwen25-05b-instruct_arc1k_evaluations.json"
    output_file = f"scores/arc_scores/basic/qwen_scores.jsonl"
    ...
    if args.visualize_distribution:
        ...
        if scores:
            figures_dir = "scores/arc_scores/basic/figures"
